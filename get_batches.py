...
def inputs(filename, batch_size, IMG_HEIGHT, IMG_WIDTH, num_epochs=None, capp=2000):
    ...
    with tf.name_scope('input'):
        ...
        img1, img2, edge1, edge2, flow = read_and_decode( filename_queue, IMG_HEIGHT, IMG_WIDTH )

        # Shuffle the examples and collect them into batch_size batches.
        # (Internally uses a RandomShuffleQueue.)
        # We run this in two threads to avoid being a bottleneck.
        img1s, img2s, edge1s, edge2s, flows = tf.train.shuffle_batch( [img1, img2, edge1, edge2, flow],
